...
/**
 * Based on Tutorial by NgocTri.
 * Modified by Firdous in 2018.
 */
...
public class DirectionsParser {
    ...
    public Direction parse(JSONObject jObject) {
            ...
            for (int i = 0; i < jRoutes.length(); i++) {
                ...
                for (int j = 0; j < jLegs.length(); j++) {
                    ...
                    for (int k = 0; k < jSteps.length(); k++) {
                        ...
                        totalTime +=(int) ((JSONObject) ((JSONObject) jSteps.get(k)).get("distance")).get("value");
                        String duration = ((JSONObject) ((JSONObject) jSteps.get(k)).get("distance")).get("text").toString();
                        String direction = ((JSONObject) jSteps.get(k)).get("html_instructions").toString();
                        directions.add(duration+ " - " +direction);
        ...
        return new Direction(routes, directions, totalTime);
    }
